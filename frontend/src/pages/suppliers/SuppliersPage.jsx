...
const SuppliersPage = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setSuppliers(data.data); setTotal(data.total);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      try {
        if (editItem) {
          await axiosInstance.put(`/suppliers/${editItem._id}`, values);
  ...
  const handleDelete = async () => {
    try { await axiosInstance.delete(`/suppliers/${deleteId}`); toast.success('Deleted'); setDeleteId(null); fetchData(); }
  ...
  return (
    <Box>
      <PageHeader title="Suppliers" subtitle="Manage your suppliers" onAdd={canWrite ? openAdd : undefined} addLabel="Add Supplier" />
      <Box sx={{ display: 'flex', gap: 2, mb: 2 }}>
        <TextField size="small" placeholder="Search suppliers..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(0); }} InputProps={{ startAdornment: <InputAdornment position="start"><SearchIcon /></InputAdornment> }} sx={{ minWidth: 280 }} />
      </Box>
      {error && <Alert severity="error" sx={{ mb: 2 }}>{error}</Alert>}
      <Box sx={{ height: 520, width: '100%' }}>
        <DataGrid rows={suppliers} columns={columns} getRowId={(r) => r._id} loading={loading} rowCount={total} pageSizeOptions={[5, 10, 25, 50]} paginationModel={{ page, pageSize }} onPaginationModelChange={(m) => { setPage(m.page); setPageSize(m.pageSize); }} paginationMode="server" disableRowSelectionOnClick sx={{ borderRadius: 2, bgcolor: 'background.paper' }} />
      </Box>
      ...
      <ConfirmDialog open={!!deleteId} title="Delete Supplier" message="Are you sure?" confirmLabel="Delete" onConfirm={handleDelete} onCancel={() => setDeleteId(null)} severity="error" />
    </Box>
  );
};
